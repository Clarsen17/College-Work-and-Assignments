...
int prod_sum(int source[], int result[], int num_values) {
    if (num_values < 2) {
        return 0; // Return 0 if the number of values is less than 2
    }
    ...
    for (int i = 0; i < num_values; i++) {
        ...
        for (int j = 0; j < num_values; j++) {
            if (j != i) {
                sum += source[j]; // Sum all elements except the corresponding value
            }
        }

        result[i] = source[i] * sum; // Calculate and assign the product
    }
